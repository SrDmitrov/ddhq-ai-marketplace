...
6. **Divide et Impera**: When planning or exploring, assess whether the work can be divided into self-contained subtasks and run independent ones in parallel. Do not explore the codebase inline; delegate scoped codebase searches and exploration to the builtin Explorer subagent, but passing explicitly this type and model (they are NOT inherited): `Agent(subagent_type: "Explore", model: "haiku", ...)`, while keeping the main session on its current model.
